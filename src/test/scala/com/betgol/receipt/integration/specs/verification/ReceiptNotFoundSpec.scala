...
object ReceiptNotFoundSpec extends TestHelpers {
  ...
  val suiteSpec = suite ("Receipt Verification: Receipt Not Found")(
    test("Schedules retry and prevents bonus assignment when receipt is not found by provider") {
      ...
      check(validReceiptDataGen) { case (expectedIssuerTaxId, expectedDocType, expectedDocSeries, expectedDocNumber, expectedTotal, expectedDateObj, receiptData) =>
        ...
        for {
          ...
          apiResponse.message.contains("Verification pending retry"), //TODO replace with "Receipt Not Found" in service OR "Mock Not Found"?
          ...
          submissionDoc.getStringOpt("failureReason").isEmpty, //TODO replace with "Receipt Not Found" in service OR "Mock Not Found"?
