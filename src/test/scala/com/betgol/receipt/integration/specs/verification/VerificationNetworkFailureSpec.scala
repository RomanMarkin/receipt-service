...
object VerificationNetworkFailureSpec extends TestHelpers {
  ...
  val suiteSpec = suite("Receipt Verification: Provider Network Failure")(

    test("Schedules retry and returns 'VerificationPending' when provider fails (Network Error)") {
      val validReceiptDataGen = for {
        issuerTaxId <- Gen.stringN(11)(Gen.numericChar)
        docType = "01"
        docSeries <- Gen.elements("F008", "B008")
      ...
      check(validReceiptDataGen) { case (expectedIssuerTaxId, expectedDocType, expectedDocSeries, expectedDocNumber, expectedTotal, expectedDateObj, receiptData) =>
        ...
        for {
          ...
          firstVerificationAttemptOpt.flatMap(_.getStringOpt("description")).contains("All providers failed. Last error: Mock Network Error"),
