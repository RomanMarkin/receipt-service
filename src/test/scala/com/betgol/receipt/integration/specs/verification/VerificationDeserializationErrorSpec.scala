...
object VerificationDeserializationErrorSpec extends TestHelpers {

  private val layer = TestSuiteLayer.make(
    MockVerificationService.deserializationErrorPath,
    MockBonusService.bonusAssignedPath,
  )

  val suiteSpec = suite("Receipt Verification: Data Deserialization Failure")(

    test("Permanently rejects submission (no retry) when request deserialization fails (Fatal Error)") {
      val validReceiptDataGen = for {
        issuerTaxId <- Gen.stringN(11)(Gen.numericChar)
        docType = "01"
        docSeries <- Gen.elements("F007", "B007")
      ...
      check(validReceiptDataGen) { case (expectedIssuerTaxId, expectedDocType, expectedDocSeries, expectedDocNumber, expectedTotal, expectedDateObj, receiptData) =>
        ...
        for {
          ...
          firstVerificationAttemptOpt.flatMap(_.getStringOpt("description")).contains("All providers failed. Last error: Mock Deserialization Error"),
