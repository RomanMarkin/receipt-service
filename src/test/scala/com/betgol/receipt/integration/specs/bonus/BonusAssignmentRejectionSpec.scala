...
object BonusAssignmentRejectionSpec extends TestHelpers {
  ...
  val suiteSpec = suite ("Bonus Assignment: Bonus Assignment Rejected")(
    test("Completes workflow with 'BonusAssignmentRejected' status when bonus provider rejects eligibility"){
      val validReceiptDataGen = for {
        issuerTaxId <- Gen.stringN(11)(Gen.numericChar)
        docType = "01"
        docSeries <- Gen.elements("F011", "B011")
