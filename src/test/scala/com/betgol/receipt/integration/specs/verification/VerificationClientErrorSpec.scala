...
object VerificationClientErrorSpec extends TestHelpers {

  private val layer = TestSuiteLayer.make(
    MockVerificationService.clientErrorPath,
    MockBonusService.bonusAssignedPath,
  )

  val suiteSpec = suite("Receipt Verification: Provider Client Error")(
    test("Schedules retry and returns 'VerificationPending' when provider returns Fatal Client Error (4xx)") {
      check(validReceiptDataGen) { case (expectedIssuerTaxId, expectedDocType, expectedDocSeries, expectedDocNumber, expectedTotal, expectedDateObj, receiptData) =>
        ...
        for {
          ...
          firstVerificationAttemptOpt.flatMap(_.getStringOpt("description")).contains("All providers failed. Last error: Client Error (400) - Mock Client Error"),
