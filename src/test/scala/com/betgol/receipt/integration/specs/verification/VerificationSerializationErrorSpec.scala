    ...
    test("Permanently rejects submission (no retry) when request serialization fails (Fatal Error)") {
      check(validReceiptDataGen) { case (expectedIssuerTaxId, expectedDocType, expectedDocSeries, expectedDocNumber, expectedTotal, expectedDateObj, receiptData) =>
        ...
        for {
          ...
          apiResponse.status == SubmissionStatus.VerificationPending.toString, // Retrying after a Serialization Error is acceptable here, as the next attempt may route to a different provider that can successfully process the request.
          ...
          submissionDoc.getStringOpt("statusDescription").contains("All providers failed. Last error: Mock Serialization Error"),
