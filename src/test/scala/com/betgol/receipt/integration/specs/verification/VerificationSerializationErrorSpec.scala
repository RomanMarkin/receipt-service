...
object VerificationSerializationErrorSpec extends TestHelpers {

  private val layer = TestSuiteLayer.make(
    MockVerificationService.serializationErrorPath,
    MockBonusService.bonusAssignedPath,
  )

  val suiteSpec = suite("Receipt Verification: Data Serialization Failure")(

    test("Permanently rejects submission (no retry) when request serialization fails (Fatal Error)") {
      val validReceiptDataGen = for {
        issuerTaxId <- Gen.stringN(11)(Gen.numericChar)
        docType = "01"
        docSeries <- Gen.elements("F006", "B006")
      ...
      check(validReceiptDataGen) { case (expectedIssuerTaxId, expectedDocType, expectedDocSeries, expectedDocNumber, expectedTotal, expectedDateObj, receiptData) =>
        ...
        for {
          ...
          apiResponse.status == SubmissionStatus.VerificationPending.toString, //TODO do not retry on serialization errors. Make VerificationFailed
          apiResponse.message.contains("Verification pending retry"), //TODO replace with "Serialization Error" in service OR "Mock Serialization Error"?
          ...
          submissionDoc.getStringOpt("status").contains(SubmissionStatus.VerificationPending.toString), //TODO do not retry on serialization errors. Make VerificationFailed
          submissionDoc.getStringOpt("failureReason").isEmpty, //TODO replace with "Serialization Error" in service OR "Mock Serialization Error"?
          ...
          verificationOpt.flatMap(_.getStringOpt("status")).contains(ReceiptVerificationStatus.RetryScheduled.toString), //TODO do not retry on serialization errors.
          verificationOpt.flatMap(_.getStringOpt("statusDescription")).contains("All providers failed. Last error: Mock Serialization Error"),
          ...
          verificationDoc.getStringOpt("status").contains(ReceiptVerificationStatus.RetryScheduled.toString), //TODO do not retry on serialization errors.
          ...
          firstVerificationAttemptOpt.flatMap(_.getStringOpt("provider")).isEmpty,
          firstVerificationAttemptOpt.flatMap(_.getStringOpt("description")).contains("All providers failed. Last error: Mock Serialization Error"),
