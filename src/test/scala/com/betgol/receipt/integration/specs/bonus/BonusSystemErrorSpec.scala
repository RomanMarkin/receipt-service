...
object BonusSystemErrorSpec extends TestHelpers {
  ...
  val suiteSpec = suite ("Bonus Assignment: Provider System Error")(
    test("Schedules retry and returns 'BonusAssignmentPending' when provider fails (System Error)"){
      val validReceiptDataGen = for {
        issuerTaxId <- Gen.stringN(11)(Gen.numericChar)
        docType = "01"
        docSeries <- Gen.elements("F013", "B013")
