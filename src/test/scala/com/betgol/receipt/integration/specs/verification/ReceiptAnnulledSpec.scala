...
object ReceiptAnnulledSpec extends TestHelpers {

  private val layer = TestSuiteLayer.make(
    MockVerificationService.docAnnulledPath,
    MockBonusService.bonusAssignedPath
  )

  val suiteSpec = suite ("Receipt Verification: Receipt Annulled")(
    test("Permanently rejects submission and prevents bonus assignment when provider reports Annulled document") {
      check(validReceiptDataGen) { case (expectedIssuerTaxId, expectedDocType, expectedDocSeries, expectedDocNumber, expectedTotal, expectedDateObj, receiptData) =>
        ...
        for {
          ...
          apiResponse.status == SubmissionStatus.VerificationRejected.toString,
          apiResponse.message.contains("Mock Annulled"),
          ...
          verificationOpt.flatMap(_.getStringOpt("status")).contains(ReceiptVerificationStatus.Annulled.toString),
          verificationOpt.flatMap(_.getStringOpt("statusDescription")).contains("Mock Annulled"),
          ...
          verificationDoc.getStringOpt("status").contains(ReceiptVerificationStatus.Annulled.toString),
          verificationDoc.getStringOpt("nextRetryAt").isEmpty,
          ...
          firstVerificationAttemptOpt.flatMap(_.getStringOpt("status")).contains(ReceiptVerificationAttemptStatus.Annulled.toString),
          ...
          firstVerificationAttemptOpt.flatMap(_.getStringOpt("description")).contains("Mock Annulled"),
